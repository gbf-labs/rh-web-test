...
class Message():
    ...
    def parse_labels(self, headers):
        if re.search(r'X-GM-LABELS \(([^\)]+)\)', str(headers)):
            labels = re.search(r'X-GM-LABELS \(([^\)]+)\)', str(headers)).groups(1)[0].split(' ')
            return map(lambda l: l.replace('"', '').decode("string_escape"), labels)
        else:
            return list()

    def parse_subject(self, encoded_subject):
        dh = decode_header(str(encoded_subject))
    ...
    def parse(self, raw_message):
        ...
        self.message = email.message_from_string(str(raw_email, 'UTF8'))
        ...
        if re.search(r'X-GM-THRID (\d+)', str(raw_headers)):
            self.thread_id = re.search(r'X-GM-THRID (\d+)', str(raw_headers)).groups(1)[0]
        if re.search(r'X-GM-MSGID (\d+)', str(raw_headers)):
            self.message_id = re.search(r'X-GM-MSGID (\d+)', str(raw_headers)).groups(1)[0]
