@update_alarm_value @all @alarm_values
Feature: Update Alarm Value
  ...
  Scenario: User can create new Alarm Value
    Given a user will logged in
     ...
     Then user type "Test2 Time Delta" in name input textbox alarm value
  ...
  Scenario: User can update Alarm Value
    Given a user will logged in
     Then user will navigates to Alarm Values page
     When user in "Alarm Values" page
     Then user will find "Test2 Time Delta" in Alarm Values page
     Then user will click update button in alarm value page
     When user in "update alarm values" page
     Then user type "Update Test Time Delta" in name input textbox alarm value
     Then user type "LaboBE01" in vessel input textbox "update" alarm value
     Then user type "COREVALUES" in device input textbox "update" alarm value
     Then user type "nothing" in device type input textbox "update" alarm value
     Then user type "Clock" in module input textbox "update" alarm value
     Then user type "TimeDelta" in option input textbox "update" alarm value
     Then user type "Raw Value" in value input textbox "update" alarm value
